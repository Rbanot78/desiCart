...
const CategoryCard = ({ product }) => {
  const { images, name, price, rating, discountPercentage, title, id, category } = product;
  const { addToCompare, compareItems } = useCompare(); // Added compareItems
  const { addToCart, addToWishlist, isProductInCart, isProductInWishlist } = useApp(); // Assuming you have functions in AppContext to add products to cart and wishlist

  // State to handle the selected main image (we're assuming the first image is the best one)
  const [mainImage, setMainImage] = useState(images && images.length > 0 ? images[0] : '/default-product.jpg');
      ...
      <Link href={`/product/${id}`}>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center mt-2">
            {discountPercentage ? (
              <>
                <p className="text-sm sm:text-lg font-bold text-blue-500 line-through mr-2">${price}</p>
                <p className="text-sm sm:text-lg font-bold text-red-600">${discountedPrice.toFixed(2)}</p>
              </>
            ) : (
              <p className="text-sm sm:text-lg font-bold text-blue-500">${price}</p>
            )}
          </div>
          ...
          <div className="flex items-center mt-2">
            <div className="flex text-yellow-500">
              {[...Array(5)].map((_, index) => (
                <FaStar key={index} className={`h-4 w-4 sm:h-5 sm:w-5 ${index < rating ? 'text-yellow-500' : 'text-gray-300'}`} />
              ))}
            </div>
